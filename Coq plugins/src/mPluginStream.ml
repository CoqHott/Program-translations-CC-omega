...
let translator : MTranslateStream.translator ref =
...
let trans_translate_constant trans cst ids =
  ...
  let fctx = { MTranslateStream.trans = trans; MTranslateStream.translator = !translator } in
  let (typ,sigma) = MTranslateStream.translate_type env fctx sigma typ in
  ...
  let (body, sigma) = MTranslateStream.translate env fctx sigma body in
...
let trans_translate (timesBool, pairTrue) gr ids =
  ...
  let trans = {
      MTranslateStream.timesBool = Nametab.global timesBool;
      MTranslateStream.pairTrue = Nametab.global pairTrue;
  } in
...
let trans_implement (timesBool, pairTrue) id typ idopt =
  ...
  let fctx = { MTranslateStream.trans = trans; MTranslateStream.translator = !translator } in
  let (typ_,sigma) = MTranslateStream.translate_type env fctx sigma typ in
  let hook _ dst =
...
let _ = register_handler
	begin function
	| MTranslateStream.MissingGlobal gr ->
