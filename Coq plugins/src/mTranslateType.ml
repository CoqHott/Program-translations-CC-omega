...
let rec translate env fctx sigma c =
  ...
  | Sort s ->
     let sigma, timesBool = Evarutil.new_global sigma trans.timesBool in
     let sigma, pairTrue = Evarutil.new_global sigma trans.pairTrue in
     (mkApp (pairTrue, [| mkApp (timesBool, [| c |]) |]), sigma)
  | Cast (c, k, t) ->
  ...
  | Prod (na, t, u) ->
     ...
     let sigma, translator = Evarutil.new_global sigma trans.pairTrue in
     (mkApp (translator, [| mkProd (na, mt, mu) |]), sigma)
  | Lambda (na, a, t) ->
...
and translate_type env fctx sigma t =
  ...
  mkProj (Projection.make (Constant.make1 (KerName.make2 mp (Label.make "fst"))) false, mt), sigma
...
and translate_ind env fctx sigma ind i k args =
  ...
  let sigma, translator = Evarutil.new_global sigma fctx.trans.pairTrue in
  if ("eq" = name) then
    match args with
    | [| t; a; b |] ->
       let mt, sigma = translate_type env fctx sigma t in
       let ma, sigma = translate env fctx sigma a in
       let mb, sigma = translate env fctx sigma b in
         (mkApp (translator, [| mkApp(mkIndU ((ind, i), k), [| mt; ma; mb |]) |]), sigma)
    | _ -> assert false
    ...
    | [| a; b |] ->
       ...
       let mb, sigma = translate_type (push_rel (na, None, a) env) fctx sigma b' in
       (mkApp (translator, [| mkApp(mkIndU ((ind, i), k), [| ma; mkLambda (na, ma, mb) |]) |]), sigma)
    | _ -> assert false
  else if ("prod" = name) then
    match args with
    | [| a; b |] ->
       let ma, sigma = translate_type env fctx sigma a in
       let mb, sigma = translate_type env fctx sigma b in
       (mkApp (translator, [| mkApp(mkIndU ((ind, i), k), [| ma; mb |]) |]), sigma)
    | _ -> assert false
  else
    match args with
    | [| |] ->
       (mkApp (translator, [| mkIndU ((ind, i), k) |]), sigma)
    | _ -> Errors.anomaly (Pp.str "translation of this inductive not implemented")
