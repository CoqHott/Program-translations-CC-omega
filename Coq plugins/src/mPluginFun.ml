...
let translator : MTranslateFun.translator ref =
...
let translate_constant cst ids =
  ...
  let typ, sigma = MTranslateFun.translate_type env !translator sigma typ in
  ...
  let body = match (Global.body_of_constant cst) with
    | Some body -> body
    | _ -> Errors.anomaly (Pp.str "Are you trying to translate some axiom?") in
  let (body, sigma) = MTranslateFun.translate env !translator sigma body in
...
let translate gr ids =
  ...
  let ans = match gr with
    ...
    (* | IndRef ind -> translate_ind ind ids *)
...
let implement id typ idopt =
  ...
  let (typ_, sigma) = MTranslateFun.translate_type env !translator sigma typ in
  let hook _ dst =
...
let _ = register_handler
	begin function
	| MTranslateFun.MissingGlobal gr ->
